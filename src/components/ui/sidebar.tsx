    ...
    if (collapsible === "none") {
      return (
        <div
          ...
          style={cssVars}
          className={cn(
            "flex h-full w-[--sidebar-width] flex-none shrink-0 flex-col bg-sidebar text-sidebar-foreground",
            className
          )}
          {...props}
        >
...
>(({ className, onClick, style, ...props }, ref) => {
  ...
  // Posiciona el botón:
  // - Abierto: dentro del sidebar, esquina superior derecha (con padding de ~0.5rem)
  // - Cerrado: esquina superior izquierda de la pantalla
  // - Mobile: siempre esquina superior izquierda
  const left = isMobile
    ? "0.5rem"
    : isOpen
